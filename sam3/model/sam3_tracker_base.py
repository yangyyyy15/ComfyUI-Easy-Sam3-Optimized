...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        # 👇 ----------- 新增：动态分辨率自适应与自愈机制 ----------- 👇
        cur_embed_size = backbone_features.size(2)
        if cur_embed_size != self.sam_image_embedding_size:
            self.sam_image_embedding_size = cur_embed_size
            self.image_size = cur_embed_size * self.backbone_stride
